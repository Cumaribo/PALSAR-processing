...
files1 <- list.files(".", "sl_HH$")#_F02DAR$")
files2 <- list.files(".", "sl_HV$")#_F02DAR$")
...
rm(r.list1, r.list2)                                                                                                                                                        
merged <- stack(merged1, merged2)                                                                                                                                                       msk<-raster('/storage/share/Mekong_ARD/Pheno_C/2000_Mekong_ard.tif')
merged <- crop(merged, extent(msk)))                                                                                                                                                    writeRaster(merged, paste('palsar', year, sep='_'), format='GTiff',overwrite=TRUE)  
#######################################
